...
def add_data_args(parser):
	...
	parser.add_argument('-limit-max-dssz', type=int, default=-1)
...
class SupervisedDataset(Dataset):
	...
	def limit_sz(self, new_sz):
		new_egs = []
		chosen_idxs = np.random.choice(total_samples, new_sz, replace=False)
		new_eg = [self.examples[i_] for i_ in chosen_idxs]
		self.all_examples = self.examples
		self.examples = new_eg
...
class MNLIDataset(Dataset):
	...
	def limit_sz(self, new_sz):
		new_egs = []
		chosen_idxs = np.random.choice(len(self), new_sz, replace=False)
		new_eg = [self.examples[i_] for i_ in chosen_idxs]
		self.all_examples = self.examples
		self.examples = new_eg
...
class DataOptions(object):
	...
	def limit_max_dataset_sz(self, max_ratio):
		lens = self.get_dataset_lens()
		ratios = np.array(list(lens.values()))
		ratios = ratios / min(ratios)
		for k, v in zip(lens.keys(), ratios):
			if v > max_ratio:
				max_sz = int(max_ratio * min(lens.values()))
				self.id_to_dataset_dict[k].limit_sz(max_sz)

	def get_dataset_lens(self):
		ds_to_len_dict = defaultdict(int)
		for k, v in self.id_to_dataset_dict.items():
			ds_to_len_dict[k] = len(v)
		return ds_to_len_dict
